...
export class ActionsBase<T extends { id: PropertyKey }> {
  ...
  constructor(protected readonly _storeKey: string) {
    this._type = (action: string): string =>
      `[${this._storeKey.toLocaleUpperCase()}] ${action}`;

    this.getByQuery = createAction(
      this._type('get by query'),
      props<ActionInit>()
    );
    this.getByQueryIsLoaded = createAction(
      this._type('get by query is loaded'),
      props<ActionSkip>()
    );
    ...
    this.getById = createAction(
      this._type('get by id'),
      props<ActionInit<{ item: Pick<T, 'id'> }>>()
    );
    this.getByIdIsLoaded = createAction(
      this._type('get by id is loaded'),
      props<ActionSkip>()
    );
    ...
    this.getByIdError = createAction(
      this._type('get by id error'),
      props<ActionError<{ error: HttpErrorResponse }>>()
    );

    this.create = createAction(
      this._type('create'),
      props<ActionInit<{ item: Omit<T, 'id'> }>>()
    );
    this.createSuccess = createAction(
      this._type('create success'),
      props<ActionSuccess<{ item: T; config?: Params; before?: Params }>>()
    );
    this.createError = createAction(
      this._type('create error'),
      props<ActionError<{ error: HttpErrorResponse }>>()
    );

    this.set = createAction(
      this._type('set'),
      props<ActionInit<{ items: T[] }>>()
    );
    this.setSuccess = createAction(
      this._type('set success'),
      props<ActionSuccess<{ items: T[] }>>()
    );

    this.update = createAction(
      this._type('update'),
      props<ActionInit<{ item: AtLeastDeep<T, 'id'> }>>()
    );
    this.updateSuccess = createAction(
      this._type('update success'),
      props<ActionSuccess<{ item: T; config?: Params; before?: Params }>>()
    );
    this.updateError = createAction(
      this._type('update error'),
      props<ActionError<{ error: HttpErrorResponse }>>()
    );

    this.updateAll = createAction(
      this._type('update all'),
      props<ActionInit<{ items: AtLeastDeep<T, 'id'>[] }>>()
    );
    ...
    this.deleteError = createAction(
      this._type('delete error'),
      props<ActionError<{ error: HttpErrorResponse }>>()
    );
  }
}
